...
export async function performLogin() {
    ...
    if (!clerkFrontendApi || !clientId) {
        throw new Error(
            'CLERK_FRONTEND_API or CLERK_OAUTH_CLIENT_ID environment variable is not set.',
        );
    }
        ...
        const server = Bun.serve({
            ...
            async fetch(req) {
                ...
                if (error) {
                    ...
                    return new Response(`Authentification failed: ${msg}`, { status: 400 });
                }
                ...
                try {
                    ...
                    setTimeout(() => server.stop(), 500);
                    return new Response(`Athentification failed: ${MessageEvent}`, { status: 400 });
                }
            },
        });
